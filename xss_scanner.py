...
def scan_url(url: str, *, session: Optional[requests.Session] = None,
    ...
    if not url.startswith(("http://", "https://")):
        return _result("xss", url, "error", errors=["URL must use HTTP or HTTPS."])
    ...
    try:
        ...
        return _result("xss", url, "error", errors=[str(exc)])

    for parameter in sorted(set(_discover_query_parameters(response.url))):
    ...
    for action, method, fields in _discover_forms(response.url, response.text):
        findings.extend(
    ...
    return _result("xss", url, "success", list(unique.values()))
...
def run_xss_scan(target: str, recon_data: Optional[Dict[str, Any]] = None,
    ...
    if not urls:
        return _result(
            "xss", target, "skipped",
            errors=["No applicable web service was discovered by Recon."]
        )
    ...
    while queue and len(visited) < max_pages:
        ...
        try:
            response = session.get(
                current, timeout=timeout, allow_redirects=True
            )
            for link in _discover_links(current, response.text):
    ...
    return _result("xss", target, status, list(unique.values()), errors)
